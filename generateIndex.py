...
# OR recursively generate 
# index.html files for 
# all subdirectories in a directory tree.

# handles symlinked files and folders 
# symlinks displayed as custom icons
...
def process_dir(top_dir, opts):
    ...
    try:
        ...
        print('WARN cannot create file %s %s' % (index_path, e))
        return
...
""")
    ...
    for entry in sorted_entries:
        ...
        # From Python 3.6 onward, os.access() accepts path-like objects
        if (not entry.is_symlink()) and not os.access(str(entry), os.W_OK):
            print(f"*** WARNING *** entry {entry.absolute()} is not writable, therefore skip.")
            continue
        if opts.verbose:
            print(f'{entry.absolute()}')

        size_bytes = -1  ## becauase it is a folder
        size_pretty = '&mdash;'
        last_modified = '-'
        ...
        try:
            ...
            print('ERROR filename not accessible:', e, entry)
            continue
...
if __name__ == "__main__":
    ...
    Generate an index file (index.html) of a directory 
    Options:
    -r, --recursive (turned off by default)
    -f, --filter by file type (i.e., --filter "*.iso")
    -v, --verbose this option will print every file that is processed. Use with caution when processing large file trees because it can cause lag and make processing slow. With smaller directories this effect should be negligible. This option is turned off by default.
    Indexing begins at current directory. However, if a folder is passed as the first positional argument, indexing will in that case begin from the folder which was passed instead of the current directory. ''')
    ...
    parser.add_argument('--recursive', '-r',
                        action='store_true',
                        help="recursively process nested dirs (set to FALSE by default)",
                        required=False)

    parser.add_argument('--verbose', '-v',
                        action='store_true',
                        help='***WARNING: very slow if file tree is large. Use with caution.***'
